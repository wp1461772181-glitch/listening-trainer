...
c.on('ready', () => {
  c.exec(`curl -s -X POST -H "Content-Type: application/json" \\
    ...
    stream.on('close', () => {
      ...
      // Regenerate blanks
      c.exec(`curl -s -X POST -H "Authorization: Bearer ${token}" http://localhost:8080/api/lessons/33/regenerate-blanks`, (err2, stream2) => {
        let body = '';
        ...
        stream2.on('close', () => {
          ...
          for (const s of l.sentences || []) {
            ...
            if (b.length > 0) {
              ...
              console.log(`  idx=${String(s.index).padStart(2)} blanks=${b.length} words=${JSON.stringify(words)}  text=${(s.text||'').substring(0,65)}`);
            }
          }
