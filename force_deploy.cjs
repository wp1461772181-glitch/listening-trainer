...
conn.on('ready', () => {
  ...
  const commands = [
    'cd /root/listening-trainer && git stash pop 2>&1 || true',
    'cd /root/listening-trainer && git fetch origin java 2>&1',
    'cd /root/listening-trainer && git reset --hard origin/java 2>&1',
